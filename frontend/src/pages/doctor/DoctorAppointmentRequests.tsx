...
const DoctorAppointmentRequests: React.FC = () => {
    ...
    const handleApprove = async (id: string) => {
        try {
            ...
            if (response?.success) {
                const next = requests.filter((req) => (req.customId || req._id) !== id);
                setRequests(next);
    ...
    const handleReject = async (id: string) => {
        const appointment = requests.find(req => (req.customId || req._id) === id);
        if (appointment) {
    ...
    const confirmReject = async () => {
        ...
        try {
            const response = await appointmentService.rejectAppointment(
                selectedAppointment.customId || selectedAppointment._id,
                rejectionReason
            );
            if (response?.success) {
                const next = requests.filter((req) => (req.customId || req._id) !== (selectedAppointment.customId || selectedAppointment._id));
                setRequests(next);
                sessionStorage.setItem('doctor_requests_cache', JSON.stringify(next));
                        ...
                        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 mb-6">
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <h3 className="text-3xl font-bold text-[#00A1B0]">
                                        {filteredRequests.length}
                                    </h3>
                                </div>
                                <div className="w-16 h-16 bg-blue-50 rounded-xl flex items-center justify-center">
                                    <FaVideo className="text-blue-600" size={32} />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-6">
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#00A1B0]"
                                />
                            </div>
                        </div>
                                            ...
                                            <div className="p-4 border-b flex gap-3">
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    <p className="text-sm text-gray-600">
                                                        {request.appointmentType === 'video' ? 'Video Call' : 'Chat'}
                                                    </p>
                                                ...
                                                <div className="w-10 h-10 bg-[#00A1B0]/10 text-[#00A1B0] rounded-lg flex items-center justify-center">
                                                    {request.appointmentType === 'video' ? (
                                                        <FaVideo size={18} />
                                                    ) : (
                                                        <FaComments size={18} />
                                                    )}
                                            ...
                                            <div className="p-4 bg-gray-50">
                                                <div className="grid grid-cols-2 text-sm mb-4">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="font-bold text-[#00A1B0]">
                                                ...
                                                <div className="flex gap-2">
                                                    <button
                                                        onClick={() => handleApprove(request.customId || request._id)}
                                                        className="flex-1 bg-green-50 hover:bg-green-100 text-green-600 py-2 rounded-lg flex items-center justify-center gap-2"
                                                    >
                                                        <FaCheck />
                                                        Approve
                                                    </button>
                                                    ...
                                                    <button
                                                        onClick={() => handleReject(request.customId || request._id)}
                                                        className="flex-1 bg-red-50 hover:bg-red-100 text-red-600 py-2 rounded-lg flex items-center justify-center gap-2"
                                                    >
                                                        <FaTimes />
                                                        Reject
                                                    </button>
                    ...
                    <div className="bg-white rounded-xl p-6 max-w-md w-full mx-4">
                        ...
                        {selectedAppointment && (
                            <div className="mb-4 p-3 bg-gray-50 rounded-lg">
                                <p className="text-sm text-gray-600">
                                    <span className="font-medium">Patient:</span> {selectedAppointment.patientId?.name || 'Unknown Patient'}
                                </p>
                                <p className="text-sm text-gray-600">
                        ...
                        <div className="mb-4">
                            ...
                            <textarea
                                ...
                                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#00A1B0] focus:border-transparent resize-none"
                                rows={4}
                            />
                        </div>
