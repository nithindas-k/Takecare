                        ...
                        {loading ? (
                            ...
                            <>
                                <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex items-center gap-4">
                                    <div className="w-12 h-12 bg-[#00A1B0]/10 rounded-2xl flex items-center justify-center text-[#00A1B0]"><FaMoneyBillWave size={20} /></div>
                                    <div>
                                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Available Balance</p>
                                        <h2 className="text-2xl font-bold text-gray-900 tracking-tight">₹{balance.toLocaleString()}</h2>
                                    </div>
                                </div>
                                <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm border-l-4 border-l-emerald-500">
                                    ...
                                    <h2 className="text-2xl font-bold text-gray-900 tracking-tight">+₹{sessionEarnings.toLocaleString()}</h2>
                                </div>
                                <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm border-l-4 border-l-rose-500">
                                    ...
                                    <h2 className="text-2xl font-bold text-gray-900 tracking-tight">-₹{sessionDeductions.toLocaleString()}</h2>
                                </div>
                            </>
                        )}
                    </div>
                                    ...
                                    {loading ? (
                                        [1, 2, 3, 4, 5].map(i => (
                                            <tr key={i}>
                                                ...
                                                <td className="px-8 py-6 text-center"><Skeleton className="h-6 w-16 rounded-full mx-auto" /></td>
                                                <td className="px-8 py-6 text-right"><Skeleton className="h-4 w-16 ml-auto" /></td>
                                            </tr>
                                        ))
                                    ) : transactions.length === 0 ? (
                                        <tr><td colSpan={5} className="px-8 py-10 text-center text-gray-500 text-sm">No transactions found for the selected criteria.</td></tr>
                                    ) : (
                                        transactions.map((txn) => {
                                            const { date, time } = formatDate(txn.createdAt);
                                            return (
                                                <tr key={txn._id} className="hover:bg-gray-50/50 transition-colors">
                                                    <td className="px-8 py-6"><span className="text-xs font-black text-gray-300 uppercase tracking-widest">#{txn._id.slice(-8).toUpperCase()}</span></td>
                                                    <td className="px-8 py-6"><div className="flex flex-col"><span className="text-sm font-bold text-gray-800 tracking-tight">{txn.description}</span><p className="text-[10px] text-gray-400 font-bold mt-1 uppercase tracking-wide">{date} • {time}</p></div></td>
                                                    <td className="px-8 py-6"><p className="text-xs font-bold text-gray-600 truncate max-w-[150px]">{txn.type}</p></td>
                                                    <td className="px-8 py-6 text-center"><span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border ${getStatusStyles(txn.status)}`}>{txn.status}</span></td>
                                                    <td className="px-8 py-6 text-right whitespace-nowrap"><span className={`text-sm font-bold tracking-tight ${txn.amount >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>{txn.amount >= 0 ? '+' : '-'}₹{Math.abs(txn.amount).toLocaleString()}</span></td>
                                                </tr>
                                            );
                                        })
                                    )}
                        ...
                        {totalPages > 1 && (
                            <div className="px-8 py-5 border-t border-gray-50 bg-gray-50/20 flex items-center justify-between">
                                ...
                                <div className="flex gap-2">
                                    <button onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))} disabled={currentPage === 1} className="w-8 h-8 flex items-center justify-center rounded-lg border border-gray-200 bg-white text-gray-500 disabled:opacity-50 disabled:cursor-not-allowed hover:border-[#00A1B0] hover:text-[#00A1B0] transition-colors"><FaChevronLeft size={10} /></button>
                                    <button onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages} className="w-8 h-8 flex items-center justify-center rounded-lg border border-gray-200 bg-white text-gray-500 disabled:opacity-50 disabled:cursor-not-allowed hover:border-[#00A1B0] hover:text-[#00A1B0] transition-colors"><FaChevronRight size={10} /></button>
                                </div>
                            </div>
                        )}
                    </div>
